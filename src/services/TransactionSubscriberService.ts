...
import { ITransactionDTO, TTransactionStatus } from '@api';

const DEFAULT_SLEEP_TIME_MS = 10_000; // 10 seconds

export type TTxCallback = (txs: ITransactionDTO[]) => void;
...
export class TransactionSubscriberService {
  ...
  public async fetchTransactions(): Promise<ITransactionDTO[]> {
  ...
  private _transactionResponseToTransactionData(tx: any): ITransactionDTO {
    return {
      id: tx.id,
      status: tx.status,
